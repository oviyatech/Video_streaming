...
GTimer *timer = NULL;
char n[1024], tmpname[100];
sem_t lock;
char res[1024];
char *local;
static int f_cnt = 0, n_cnt=1, m_cnt=1;
...
/* Appending the folder Location for the target file namee*/
        snprintf(tgt, sizeof(tgt), "/oviya/%s",target);
        printf("TGT=%s\n",tgt);
...
int factory_make(char* location)
{
	snprintf(folder, sizeof(folder), "/video/%s",location);
	printf("Loaction ****************  %s\n",folder);
	timer = g_timer_new ();
	...
	g_timeout_add ((8 * 1000), (GSourceFunc) stop_capture, NULL);
...
//	g_object_set (G_OBJECT (filter), "caps", video_caps, NULL);
	...
	g_object_set (G_OBJECT (sink), "location", folder, NULL);
...
//	timer = g_timer_new ();
//	g_timer_start (timer);
//	g_timeout_add ((1 * 1000), (GSourceFunc) stop_capture, NULL);
        /* Initialize elements */
        	if(factory_make(strcat(F_name,".avi")) != 0)
                return -1;
...
void * stop_camera()
{
	...
	unsigned short id[4];
	printf("Enter stop button thread\n");
	...
	while (1){
			...
			printf("Read Size =%d %d\n", rd, sizeof(struct input_event) * 64);
				if (rd < (int) sizeof(struct input_event)) {
					printf("yyy\n");
					perror("\nevtest: error reading");
					return 1;
				}
		printf("EV type %d  EVcode %d, EV value %d\n",ev[0].type, ev[0].code, ev[0].value);
...
printf("Enter main Thread\n");
    //    int status;
        ...
        pthread_create(&tcap,NULL,start_capture,NULL);
        pthread_create(&tstp,NULL,stop_camera,NULL);

        pthread_join(tcap,NULL);
        pthread_join(tstp,NULL);
//        return ;
...
int main (int argc, char **argv)
{
	...
	printf(" SHAN CHECK ARGC=%d ARGV1=%s ARGV0=%s\n",argc,argv[argc-1], argv[0]);
	int fd, rd, i, j, k;
	struct input_event ev[64];
	int version;
	unsigned short id[4];
